...
class Map extends Component {
  constructor(props) {
    ...
    this.convertTravelTimeToMinutes = this.convertTravelTimeToMinutes.bind(
      this
    );
  }
  ...
  findLargestTravelTime() {
    ...
    for (let i = 0; i < this.state.etaResponse.length; i++) {
      if (
        this.convertTravelTimeToMinutes(this.state.etaResponse[i][0]) >
        maxTimeInt
      ) {
        maxTime = this.state.etaResponse[i][0];
        maxTimeInt = this.convertTravelTimeToMinutes(
          this.state.etaResponse[i][0]
        );
      }
    }
    return maxTime;
    ...
    if (hoveredObject && hoveredObject.message == "Your Destination") {
      return (
        hoveredObject && (
          ...
          >
            {this.state.loadedETA ? hoveredObject.travelTime : "Loading..."}
          </div>
        )
      );
    }
  ...
  callAxios = async workingGroup => {
    let response = await axios
      .post("http://localhost:4000/api/directions/eta", { workingGroup })
      .catch(err => {
        console.log(err);
      });
    this.setState({
  ...
  render() {
    if (this.props.currentGroup === -1) {
      return <div>Select a group to see the path</div>;
    }
    ...
    for (let i = 0; i < workingGroup.paths.length; i++) {
      //Get the generated api paths from the group state
      ...
      let newIcon = new IconLayer({
        id:
          workingGroup.users[i].name +
          workingGroup.users[i].id +
          workingGroup.latitude +
          workingGroup.longitude,
    ...
    destinationCoordinates.push(
      workingGroup.paths[0][workingGroup.paths[0].length - 1][1]
    );
    destinationCoordinates.push(
      workingGroup.paths[0][workingGroup.paths[0].length - 1][0]
    );
    let destinationObject = {
      ...
      travelTime:
        "All members will reach the destination in " +
        this.findLargestTravelTime()
    };
    ...
    return (
      <div>
        <div
          className="infoMessage"
          style={{ position: "absolute", zIndex: 1, left: 1350, top: 20 }}
        >
          {this.state.loadedETA ? (
            <div>
              All members will reach the destination in{" "}
